    ...
    public class PizzaOrder
    {
        ...
        public void createOrder()
        {
            ...
            do // loop to order pizza item(s)
            {
                ...
                TotalPizzaCount += pizzaOrderList[pizzaOrderList.Count - 1].numberOfPizza;
                total += pizzaOrderList[pizzaOrderList.Count - 1].itemPrice * pizzaOrderList[pizzaOrderList.Count - 1].numberOfPizza;
        ...
        public Pizza orderPizza()
        {
            ...
            pizza.numberOfPizza = selectNumberOfPizza(pizza);
            pizza.itemPrice = calculateItemPrice(pizza.size, pizza.numOfToppings);
        ...
        public void selectToppings(ref Pizza pizza)
        {
            ...
            if (tempSelection1 == 2)
            {
                ...
                for (int i = 0; i < pizza.numOfToppings; i++)
                {
                    ...
                    if (i == 0)
                    { pizza.toppings =toppingType[tempSelection]; }
                    else
                    { pizza.toppings = pizza.toppings + ", " + toppingType[tempSelection]; }
                    Console.WriteLine($"You have selected {toppingType[tempSelection]}");
                }
            }
            else if (tempSelection1 == 1)
            {
                pizza.toppings=toppingType[0]+", "+ toppingType[3];
                pizza.numOfToppings = 2;
                Console.WriteLine($"You have selected default toppings.");
            }
        }
        public void dispayItemDetail(Pizza pizza)
        {
            Console.WriteLine(" ");
            Console.WriteLine($"--------------Item #{pizza.typeID} ---------------");
            Console.WriteLine($"Pizza size: " + pizza.size);
            Console.WriteLine($"{pizza.numOfToppings} Toppings: " + pizza.toppings);
            Console.WriteLine($"Pizza Crust: {pizza.crust}");
            Console.WriteLine($"Item Price: ${pizza.itemPrice}");
            Console.WriteLine($"Number Of Pizza(s): {pizza.numberOfPizza}");
            Console.WriteLine($"Sub Total: ${pizza.numberOfPizza* pizza.itemPrice}");
            Console.WriteLine("--------------------------------------");
        }
        ...
        public int selectNumberOfPizza(Pizza pizza)
        {
            ...
            testTotal = Decimal.ToInt32((maxTotal - total) / calculateItemPrice(pizza.size, pizza.numOfToppings));
